...
def _build_feasible_customer_mask(ninf_mask):
    feasible_mask = torch.isfinite(ninf_mask)
    feasible_mask[..., 0] = False
    return feasible_mask


def _is_cvrptw_feature_view(candidate_features):
    return bool(getattr(candidate_features, "linc_cvrptw_feature_view", False))


def _feature_view_current_index(view):
    if view.current_node is None:
        return torch.zeros(
            (view.batch_size, view.rollout_size),
            dtype=torch.long,
            device=view.BATCH_IDX.device,
        )
    return view.current_node


def _feature_view_customer_features(view, dtype):
    current_idx = _feature_view_current_index(view)
    dist_to_customers = view.cand_step_customer_dist[view.BATCH_IDX, current_idx]
    travel_dist_norm = view.cand_step_travel_dist_norm[view.BATCH_IDX, current_idx]
    depot_angle_diff_norm = view.cand_step_angle_diff_norm[view.BATCH_IDX, current_idx]
    arrival = view.time[:, :, None] + dist_to_customers
    horizon_fallback = torch.maximum(
        view.cand_max_customer_due[:, None, :],
        view.time[:, :, None] + 2.0 * view.cand_dist_scale[:, None, None],
    )
    horizon = torch.where(view.cand_has_depot_due, view.cand_depot_due, horizon_fallback).clamp_min(1.0)
    inv_horizon = torch.reciprocal(horizon + 1e-6)
    wait = torch.relu(view.cand_customer_ready - arrival)
    start = torch.maximum(arrival, view.cand_customer_ready)
    tw_slack = view.cand_customer_due - start
    finish = start + view.cand_service_time
    features = {
        "travel_dist_norm": travel_dist_norm,
        "wait_norm": wait * inv_horizon,
        "tw_slack_ratio": tw_slack * inv_horizon,
        "arrival_time_norm": arrival * inv_horizon,
        "departure_time_norm": finish * inv_horizon,
        "depot_angle_diff_norm": depot_angle_diff_norm,
    }
    return {
        name: torch.nan_to_num(value.to(dtype=dtype), nan=0.0, posinf=0.0, neginf=0.0)
        for name, value in features.items()
    }


def _lookup_selected_feature(selected_phi, feature_name, local_index_map):
    local_idx = local_index_map.get(feature_name)
    if local_idx is None:
        shape = selected_phi.shape[:3]
        return selected_phi.new_zeros(shape)
...
def _build_tiny_residual_summary(
    ...
    customer_count = max(selected_phi.size(2) - 1, 1)
    ...
    if summary_mode == "full_mu":
        if selected_mean is None:
            selected_mean = _masked_mean_dim(selected_phi, feasible_customer_mask.unsqueeze(-1), dim=2)
        summary = torch.cat([feasible_ratio.unsqueeze(-1), selected_mean], dim=-1)
        return summary, feasible_ratio
    if summary_mode != "partial":
        raise ValueError(f"Unsupported qlite summary_mode: {summary_mode}")
    slack = _lookup_candidate_feature(selected_phi, candidate_features, "tw_slack_ratio", local_index_map)
    summary = torch.stack(
        [
            feasible_ratio,
            ...
            _lookup_candidate_feature_mean(
                selected_phi, candidate_features, "wait_norm", local_index_map, feasible_customer_mask, selected_mean
            ),
            _masked_min_dim(slack, feasible_customer_mask, dim=2),
        ],
        dim=-1,
    )
    return summary, feasible_ratio
...
def _build_relative_features(
    ...
    rel_out = rel_values - rel_mean

    if normalization_mode == "zscore" and zscore_index is not None and zscore_index.numel() > 0:
        zscore_global = set(zscore_index.tolist())
        zscore_pos = [pos for pos, feat_idx in enumerate(rel_index.tolist()) if feat_idx in zscore_global]
        if zscore_pos:
            zscore_pos_tensor = torch.tensor(zscore_pos, dtype=torch.long, device=selected_phi.device)
            z_vals = rel_values.index_select(dim=-1, index=zscore_pos_tensor)
            z_mask = rel_mask.to(dtype=selected_phi.dtype)
            z_mean = _masked_mean_dim(z_vals, z_mask, dim=2).unsqueeze(2)
            z_var = _masked_mean_dim((z_vals - z_mean) ** 2, z_mask, dim=2).unsqueeze(2)
            z_std = torch.sqrt(z_var + 1e-6).to(dtype=selected_phi.dtype)
            rel_out[..., zscore_pos_tensor] = (z_vals - z_mean.to(dtype=selected_phi.dtype)) / z_std

    phi_q = selected_phi.clone()
    phi_q[..., rel_index] = rel_out
...
class QuotientLiteCandidateScorer(nn.Module):
    ...
    This is the only retained quotient-style scorer in the main CVRPTW path:
    summary -> gamma / beta / alpha, then a lightweight additive candidate branch.
    """
    ...
    def __init__(
        ...
        feature_centering_mode="centered",
        summary_mode="partial",
        summary_dim=4,
        hidden_dim=None,
        activation="gelu",
        ...
        self.feature_centering_mode = str(feature_centering_mode)
        if self.feature_centering_mode not in {"centered", "raw"}:
            raise ValueError(f"Unsupported qlite feature_centering_mode: {self.feature_centering_mode}")
        self.summary_mode = str(summary_mode)
        if self.summary_mode not in {"partial", "full_mu"}:
            raise ValueError(f"Unsupported qlite summary_mode: {self.summary_mode}")
        ...
        self.full_phi_proj = nn.Linear(self.feature_dim, self.embedding_dim, bias=phi_proj_bias)
    ...
    def _materialize_feature_view(self, candidate_features, dtype):
        customer_features = _feature_view_customer_features(candidate_features, dtype)
        selected_phi = torch.zeros(
            (
                candidate_features.batch_size,
                candidate_features.rollout_size,
                candidate_features.problem_size + 1,
                self.feature_dim,
            ),
            dtype=dtype,
            device=candidate_features.BATCH_IDX.device,
        )
        for local_idx, name in enumerate(self.selected_candidate_feature_names):
            selected_phi[:, :, 1:, local_idx] = customer_features[name]
        return selected_phi

    def _forward_feature_view(self, decoder_context, candidate_key, candidate_features, ninf_mask, return_aux=False):
        selected_phi = self._materialize_feature_view(candidate_features, decoder_context.dtype)
    ...
    def _forward_tensor(self, decoder_context, candidate_key, candidate_features, ninf_mask, selected_phi=None, return_aux=True):
        ...
        if self.feature_centering_mode == "raw":
            raw_feature_weights = torch.matmul(h_tilde, self.full_phi_proj.weight)
            dynamic_score = (selected_phi * raw_feature_weights.unsqueeze(2)).sum(dim=-1)
    ...
    def forward(self, decoder_context, candidate_key, candidate_features, ninf_mask, selected_phi=None, return_aux=True):
        if _is_cvrptw_feature_view(candidate_features):
            return self._forward_feature_view(
                decoder_context,
                candidate_key,
                candidate_features,
                ninf_mask,
