...
TSP_ROOT = ROOT
LINC_ROOT = TSP_ROOT / "LINC"

if str(LINC_ROOT) not in sys.path:
    sys.path.insert(0, str(LINC_ROOT))
...
DEFAULT_MODEL_PARAMS = {
    ...
    "relative_candidate_feature_names": ["travel_dist_norm", "dist_to_start_norm", "dist_to_centroid_norm"],
...
def parse_args():
    parser = argparse.ArgumentParser(description="LINC TSP SGBS benchmark.")
    ...
    parser.add_argument("--include-routes", action="store_true", help="Store best decoded tour for each instance.")
    parser.add_argument("--output-json", type=str, default=str(ROOT / "results" / "tsp_linc_sgbs.json"))
    return parser.parse_args()


def infer_model_params_from_checkpoint(checkpoint):
    mp = dict(DEFAULT_MODEL_PARAMS)
    mp.update(dict(checkpoint.get("model_params", {})))
...
# SGBS core: env manipulation and beam search.
...
def clone_env_shallow(env):
    ...
    cloned.step_state = type(env.step_state)(**dict(env.step_state.__dict__))
    if hasattr(env, "reset_state") and env.reset_state is not None:
        cloned.reset_state = type(env.reset_state)(**dict(env.reset_state.__dict__))
    return cloned
...
def copy_dynamic_state(env):
    ...
    cloned.step_state.candidate_features = None if env.step_state.candidate_features is None else env.step_state.candidate_features.clone()
...
def activate_model_cache(model, env, z):
    ...
    cache = {
        ...
        "node_embeddings": model.decoder.node_embeddings,
        "z": model.decoder.z,
    }
...
def decode_scores(model, env, z):
    ...
    try:
        ...
        with torch.amp.autocast(device_type="cuda", dtype=torch.float16, enabled=encoded_last_node.is_cuda):
            scores = model.decoder(encoded_last_node, ninf_mask=state.ninf_mask,
                                   candidate_features=getattr(state, "candidate_features", None), return_logits=True)
        return state, scores
    finally:
    ...
    while not done:
        ...
        try:
            ...
            while not rollout_done:
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.float16, enabled=encoded_last_node.is_cuda):
                    scores = model.decoder(encoded_last_node, ninf_mask=rollout_state.ninf_mask,
                                           candidate_features=getattr(rollout_state, "candidate_features", None), return_logits=True)
                selected = scores.argmax(dim=2)
...
def run_variant(checkpoint_path, *, dataset_pkl, num_instances, index_begin, instance_batch_size,
                beam_width, expand_k, aug_factor, z_samples, seed, device, include_routes=False):
    ...
    with torch.no_grad():
        for batch_start in range(0, int(num_instances), int(instance_batch_size)):
            ...
            tours = final_env.selected_node_list[:, :, :final_env.problem_size].detach().cpu() if include_routes else None
            ...
            for local_idx in range(current_batch):
                ...
                if tours is not None:
                    aug_idx = row["aug_index"]
                    beam_idx = row["beam_index"]
                    env_row = int(aug_idx) * current_batch + int(local_idx)
                    row["tour"] = [int(node) for node in tours[env_row, beam_idx].tolist()]
                rows.append(row)
...
def main():
    ...
    result = run_variant(Path(args.ours_checkpoint).resolve(), dataset_pkl=dataset_pkl,
                         ...
                         include_routes=bool(args.include_routes))
