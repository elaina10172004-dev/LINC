...
class TSPModel(nn.Module):
    ...
    def forward(self, state, greedy_construction=False, EAS_incumbent_action=None):
        ...
        if self.force_first_move and state.current_node is None:
            selected = torch.arange(problem_size, device=device).repeat(rollout_size // problem_size)[None, :].expand(batch_size,
                                                                                                    rollout_size)
...
class TSP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, ninf_mask, return_logits: bool = False):
