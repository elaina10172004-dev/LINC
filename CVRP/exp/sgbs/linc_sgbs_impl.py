"""LINC CVRP SGBS implementation.

This module imports only ``CVRP/LINC`` code. PolyNet uses the sibling
``polynet_sgbs_impl.py`` to avoid model-path cross contamination.
"""
...
CVRP_ROOT = ROOT
CVRP_MODEL_ROOT = CVRP_ROOT / "LINC"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Benchmark LINC CVRP checkpoints with SGBS."
    )
    ...
    parser.add_argument(
        "--output-json",
        type=str,
        default=str(CVRP_ROOT / "results" / "cvrp_linc_sgbs.json"),
    )
