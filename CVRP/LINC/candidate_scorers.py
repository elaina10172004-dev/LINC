...
def _build_tiny_residual_summary(
    ...
    load_after = _lookup_candidate_feature(selected_phi, candidate_features, "load_after_ratio", local_index_map)
    summary = torch.stack(
        [
            feasible_ratio,
            ...
            _lookup_candidate_feature_mean(
                selected_phi, candidate_features, "demand_ratio", local_index_map, feasible_customer_mask, selected_mean
            ),
            _masked_min_dim(load_after, feasible_customer_mask, dim=2),
        ],
        dim=-1,
    )
...
def _linear_relative_dynamic_score(
    ...
    dynamic_score = torch.matmul(selected_phi, dynamic_feature_weights.unsqueeze(-1)).squeeze(-1)
    if relative_index.numel() > 0:
        rel_index = relative_index.to(device=selected_phi.device)
...
class QuotientLiteCandidateScorer(nn.Module):
    ...
    def _compute_score(
        ...
        base_score = alpha * torch.bmm(decoder_context, candidate_key.transpose(1, 2))
        if phi_q is None:
